...
//出入站域名匹配
const deno_domains = {
    "second.example.com": ["yyy.deno.dev","yyy.deno.dev"],
    "third.example.com": ["yyy.deno.dev","yyy.deno.dev"],
    "first.example.com": ["xxx.deno.dev","xxx.deno.dev"],
}
//网页内容替换，注意逗号
const replace_dict = {
...
async function fetchAndApply(request) {
    ...
    //判断入站域名是否在运行列表里。
    if (deno_domains.hasOwnProperty(url_host)==false){
        return new Response("Error: Invalid Domain.", { headers: { "content-type": "text/plain" }})
    }else{
    var upstream =deno_domains[url_host][0]
    var upstream_mobile = deno_domains[url_host][1]
    }
